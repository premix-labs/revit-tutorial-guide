...
| **Visual Style** | `Hidden Line` (ทำงาน), `Shaded` (เช็ก) | Hidden Line เร็วกว่า Shaded            |
...
## ถ้าติดขัด ให้ดูตรงไหนก่อน

| อาการที่เจอ | ให้ดูที่ไหนก่อน | เพราะอะไร |
| ---------- | --------------- | --------- |
| หา view ไม่เจอ | `Project Browser` | view หลักของหนังสือ เช่น `F1`, `F2`, `Roof` จะถูกเปิดจากตรงนี้ |
| แก้ค่าชิ้นส่วนไม่ได้ | `Properties` | ค่า instance และค่าของ view จะเปลี่ยนตามสิ่งที่เลือก |
| มองไม่เห็นเหล็ก | `View Control Bar` + `Properties` | มักเกิดจาก `Detail Level`, `Discipline` หรือการมองเห็นของ view |
| หาเมนูไม่เจอ | `Ribbon` | ให้ยึดชื่อคำสั่งบน Ribbon เป็นหลักก่อนคีย์ลัด |
| ซ่อน/แสดงองค์ประกอบไม่ถูก | `VV/VG` หรือ view properties | ปัญหามักมาจากการตั้งค่าการมองเห็นของ view ไม่ใช่ตัวโมเดลหาย |
| `Project Browser` หรือ `Properties` หาย | `View > User Interface` | ใช้เปิด panel ที่ถูกปิดหรือซ่อนไว้ |

> [!TIP]
> ถ้าคีย์ลัดในเครื่องคุณไม่ตรงกับหนังสือ ให้ใช้ชื่อเมนูบน Ribbon, ชื่อ command ใน `Properties` และชื่อ view ใน `Project Browser` เป็นตัวอ้างอิงหลักก่อนเสมอ
